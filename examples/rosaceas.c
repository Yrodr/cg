...
polygon * rosacea(int, int);  // rosáceas

polygon * rosacea(int a, int n) {
...
int main(int argc, char ** argv) {
  ...
  // objeto em coordenadas polares (rho,theta): rho = a*cos(n*theta)
  // n -> inteiro positivo
  // n par -> 2*n laços
  // n impar -> n laços
  objeto = CreateObject2d(4); 

  curva = rosacea(3,2);  // 4 laços
  SetColorPolig2d(1,curva);
  SetObject2d(curva,objeto);
  
  curva = rosacea(3,3); // 3 laços
  SetColorPolig2d(2,curva);
  SetObject2d(curva,objeto);
  
  curva = rosacea(2,4); // 8 laços
  SetColorPolig2d(3,curva);
  SetObject2d(curva,objeto);
  
  curva = rosacea(3,5); // 5 laços
