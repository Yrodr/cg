...
polygon * lemniscatas1(int);  // rho² = (+ ou -) a*cos(2*theta)
polygon * lemniscatas2(int);  // rho² = (+ ou -) a*sen(2*theta)

polygon * lemniscatas1(int a) {
  float step, rho, theta, x, y;
  polygon * curva;
    
  curva = CreatePolig2d(360);

  step = (2.0*M_PI)/(360.0);
  for(theta=0.0;theta<(2.0*M_PI)-step;theta+=step) {
    rho = sqrt(a*cos(2.0*theta));
    x = rho*cos(theta);
    y = rho*sin(theta);
...
int main(int argc, char ** argv) {
  ...
  // curvas em coordenadas polares (rho,theta):
  // rho² = (+ ou -) a*cos(2*theta)
  // rho² = (+ ou -) a*sen(2*theta) 
  objeto = CreateObject2d(4);

  curva = lemniscatas1(4);  // rho² = a*cos(2*theta)
  ...
  curva = lemniscatas2(4);  // rho² = a*sin(2*theta) 
