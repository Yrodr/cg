...
// Espiral de Fermat: r² = a²*θ
polygon * espiralFer(float, unsigned int);

polygon * espiralFer(float a, unsigned int n) {
...
int main(int argc, char ** argv) {
  ...
  janela = CreateWindow(-4.0,-4.0,4.0,4.0);
  ...
  // objeto em coordenadas polares (rho,theta) -> Espiral de Fermat
  // r² = a²*θ
  objeto = CreateObject2d(4);

  curva = espiralFer(1.0/M_PI,1);
  SetColorPolig2d(1,curva);
  SetObject2d(curva,objeto);
  
  curva = espiralFer(1.0/M_PI,2);
  SetColorPolig2d(2,curva);
  SetObject2d(curva,objeto);
  
  curva = espiralFer(1.0/M_PI,3);
  SetColorPolig2d(3,curva);
  SetObject2d(curva,objeto);
  
  curva = espiralFer(1.0/M_PI,4);
